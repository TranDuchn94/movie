...
<%--                <jsp:include page="../fragment/aside-left.jsp"/>--%>
        ...
        <div class="col-sm-8 col-md-9 col-xl-10 bg-white pb-3">
            <div class="p-2">
                <h1 class="pt-3 pb-3 border-bottom border-muted mb-3">Employee Search List by <%=filter.toUpperCase()%></h1>
                ...
                <table id="employee-table" class="table table-striped table-bordered">
                    ...
                    <tr>
                        ...
                        <td>
                            <a class="text-primary text-decoration-none" href="${pageContext.request.contextPath}/employees/view?id=<%=employee.getId()%>&pageNumber=<%=pageNumber%>&keyword=<%=keyword%>&filter=<%=filter%>"><i class="fa-solid fa-eye"></i>
                                View</a> |
                            <a class="text-warning text-decoration-none"
                               href="${pageContext.request.contextPath}/employees/update?id=<%=employee.getId()%>&pageNumber=<%=pageNumber%>&keyword=<%=keyword%>&filter=<%=filter%>"><i
                                    class="fas fa-edit"></i> Update</a> |
                            <a class="text-danger text-decoration-none"
                               href="${pageContext.request.contextPath}/employees/delete?id=<%=employee.getId()%>&pageNumber=<%=pageNumber%>&totalRecord=<%=totalRecord%>&keyword=<%=keyword%>&filter=<%=filter%>"><i
                                    class="fa-solid fa-trash-can"></i> Delete</a>
                        </td>
                    </tr>
                    <% } %>
                </table>
                <%--                paging with keyword search , display all employee with filter and keyword search--%>
                <nav id="pagination" aria-label="Page navigation">
                    <ul class="pagination">
                        <% if(pageNumber > 1){ %>
                                <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/employees/search?pageNumber=<%=pageNumber-1%>&keyword=<%=keyword%>&filter=<%=filter%>">Previous</a></li>
                        <% } %>

                        <% if(totalPage <= 5){
                            for(int i = 1; i<=totalPage; i++){
                                if(i == pageNumber){%>
                                    <li class="page-item"><a style="background-color: dodgerblue;color: white" class="page-link" href="${pageContext.request.contextPath}/employees/search?pageNumber=<%=i%>&keyword=<%=keyword%>&filter=<%=filter%>"><%=i%></a></li>
                            <%} else{%>
                                    <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/employees/search?pageNumber=<%=i%>&keyword=<%=keyword%>&filter=<%=filter%>"><%=i%></a></li>
                            <%}
                            }
                            ...
                            for(int i = 0; i<5; i++){
                                if(i == 2){%>
                                    <li class="page-item"><a style="background-color: dodgerblue;color: white" class="page-link" href="${pageContext.request.contextPath}/employees/search?pageNumber=<%=pageNumber-2+i%>&keyword=<%=keyword%>&filter=<%=filter%>"><%=pageNumber-2+i%></a></li>
                            <%} else{%>
                                    <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/employees/search?pageNumber=<%=pageNumber-2+i%>&keyword=<%=keyword%>&filter=<%=filter%>"><%=pageNumber-2+i%></a></li>
                            <%}
                            }
                        }else{
                        ...
                        <% if(pageNumber < totalPage){%>
                                <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/employees/search?pageNumber=<%=pageNumber+1%>&keyword=<%=keyword%>&filter=<%=filter%>">Next</a></li>
                        <% }%>
                    </ul>
                </nav>
            </div>
